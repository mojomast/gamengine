...
class VampireSurvivorsGame extends GameEngine {
    ...
    fireWhip(weapon) {
        // Horizontal attack in front of player
    ...
    fireKnife(weapon) {
        // Fire in movement direction or right if not moving
        let angle = 0;
        if (this.keys['KeyW']) angle = -Math.PI / 2;
        else if (this.keys['KeyS']) angle = Math.PI / 2;
        else if (this.keys['KeyA']) angle = Math.PI;
        else if (this.keys['KeyD']) angle = 0;
    ...
    isColliding(projectile, enemy) {
        ...
        switch (projectile.type) {
            case 'whip':
                // Rectangle collision
                colliding = projectile.x < enemy.x + enemy.size &&
    ...
    renderProjectiles(ctx) {
        this.projectiles.forEach(projectile => {
            ...
            switch (projectile.type) {
                case 'whip':
                    ctx.fillRect(projectile.x, projectile.y - projectile.height / 2, 
...
window.startGame = function() {
    if (window.vampireGame) {
        window.vampireGame.startGame();
    }
};
...
window.returnToMenu = function() {
    if (window.vampireGame) {
        ...
        window.vampireGame.gameState = 'menu';
    }
};

// Initialize game when page loads
document.addEventListener('DOMContentLoaded', () => {
    // Note: This would normally import from the actual mojomast/gamengine
    // For this demo, we'll create a simplified version
    window.vampireGame = new VampireSurvivorsGame();
